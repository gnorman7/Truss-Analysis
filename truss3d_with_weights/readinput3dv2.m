function [joints,connectivity,reacjoints,reacvecs,loadjoints,loadvecs]=readinput3dv2(inputfile)
...
% Weights
% 1 g = 0.00220462 lbf (g=9.81)
% wJoints = 0.017 g;
% sdMembers = 5.35/5.25 g/in;
gTolbf = 0.00220462; %grams to lbf conversion factor
wJoints = 1.7*gTolbf %lb
sdMembers = (5.35/5.25)*gTolbf %lb/in
...
% At this point we have all the forces
fJoints = zeros(numjoints,1);
for i=1:numjoints
    wAllMembers = 0;
    for j = 1:numbars
                 le = 0; % length of member connected to joint i;
      if connectivity(j,1) == i || connectivity(j,2) == i
          for k = 1:3
             le = le + (joints(connectivity(j,1),k) - joints(connectivity(j,2),k))^2;
          end
          le = sqrt(le);
       end
       wAllMembers = wAllMembers + le*sdMembers/2;
    end
    
    fJoint(i) = wJoints + wAllMembers;
end
...
for i = 1:numjoints
    ...
    for j = 1:length(loadjoints)
        if loadjoints(j) == i
            loadvecs(j,3) = loadvecs(j,3) - fJoint(i);
            flag = 1;
        end
    end
    if ~flag
        loadvecs(length(loadvecs(:,1))+1,:) = [0 0 -fJoint(i)];
        loadjoints(length(loadjoints(:,1))+1,:) = i;
    end
end
